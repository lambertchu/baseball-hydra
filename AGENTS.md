...
This folder implements a **multi-task learning (MTL) neural network that predicts next-season (preseason) and rest-of-season (ROS, in-season) counting and rate stats for MLB batters** using historical performance, Statcast quality metrics, biometric data, ballpark factors, and team context.

**LLMs must follow this document as the authoritative build plan and working agreement.**
This document is the single source of truth — the same content is served at both `AGENTS.md` (canonical) and `CLAUDE.md` (symlink) so every agent harness picks it up without drift.
...
- **Weekly log sources and the 2016-2022 SB defect (fixed 2026-07)**: `batting_week_{year}.parquet` must come from BRef (`fetch_game_logs.py`, per-week retries absorb BRef throttles). The Statcast-derived fallback (`--source statcast`) cannot see mid-PA steals — it ships only ~6% of SB and ~38% of CS — and briefly poisoned 2016-2022 SB features **and** `ros_sb*` targets before those seasons were re-fetched from BRef (SB coverage now ≈ 100%; CS ≈ 81-87% vs FanGraphs is a definitional pickoff-CS gap, not data loss). `validate_weekly_counting_coverage` runs on every snapshot build and warns when any of R/RBI/SB/CS sum to < 80% of local season totals, so a lossy source can no longer reach training silently. The fallback's `--calibrate-season-totals` stays off by default (end-of-season leakage into mid-season rows).
...
### 5.2 Phase 2 ROS MTL — `src/models/mtl_ros/` (quantile regressor, parked)

**Architecture:**

- Same shared-backbone three-group head structure as the preseason MTL, but each per-target head emits 5 pinball-loss quantile outputs (τ ∈ {0.05, 0.25, 0.50, 0.75, 0.95}).
- Auxiliary 7th PA-remaining regression head (`pa_remaining`) for count-stat conversion at inference.
- `MultiTaskQuantileLoss`: per-task pinball + Kendall homoscedastic uncertainty weighting over the 6 rate tasks + MSE (or Gaussian-NLL) on the PA head, weighted by `pa_weight`.
- Multi-seed ensemble (`MTLQuantileEnsembleForecaster`) aggregates N seeds with per-quantile mean and a monotonic `np.sort` along the quantile axis.
- Trained on per-(player, season, ISO-week) cutoffs from `weekly_snapshots_{year}.parquet` via `walk_forward_split` (atomic `(mlbam_id, season)` split, season-boundary filters, post-hoc leakage assertion).
...
| **Feature Engineering**    | `src/features/*.py`                                         | Registered features across 10 groups (some excluded via config): batting, statcast, non-contact (stabilisation-regressed), speed, bat speed, age, park factors, team stats, temporal, in_season (Phase 2, opt-in) |
...
| **Weekly Snapshot Layer**  | `src/data/fetch_game_logs.py`, `build_snapshots.py`, `fetch_statcast.py` (`_aggregate_batter_statcast_weekly`), `rate_helpers.py` | Per-(player, ISO-week) BRef batting logs (per-week retries for BRef throttles) + Statcast BBE aggregates → weekly snapshots with `*_week`, `*_ytd`, `trail4w_*`, and `ros_*` columns. Raw Statcast retains `game_date`. Shared `obp_slg` / `safe_div` primitives in `rate_helpers.py` keep the ytd and trail4w rate formulas from drifting. Every build runs `validate_weekly_counting_coverage` against local season totals (§3.2). A Statcast-derived fallback (`--source statcast`) exists for BRef-less environments — accurate for PA/AB/H/HR/BB/SO, ~4% low on R/RBI, but undercounts SB/CS badly (§3.2); all of 2016-2022 is BRef-sourced as of 2026-07. |
...
- ROS benchmark: `data/reports/benchmark_ros/benchmark_ros_report.json` + `benchmark_ros_table.csv` (+ optional `pit_{baseline}.png` per distributional baseline)
- 2026 preseason projections: `data/projections/projections_mtl_2026.csv`
- 2026 ROS projections: `data/projections/ros_mtl_2026.csv`

### 7.3 Benchmark Results

**Preseason (full-season)** — pooled RMSE across 2022-2025 (4 years, 1,063 player-seasons, rolling retrain):
...
**Preseason ablation results** (which MTL features help):
...
# Phase 2 ROS MTL training (standalone)
...
```
baseball-hydra/
├── AGENTS.md                          # Canonical build plan (source of truth)
├── CLAUDE.md → AGENTS.md              # Symlink for Claude Code auto-discovery
...
│   │   ├── rate_helpers.py            # Shared obp_slg / safe_div primitives (ytd & trail4w)
...
│   │   ├── in_season.py               # Phase 2: 24-col in-season features from weekly snapshots
│   │   ├── pipeline.py                # End-to-end feature pipeline
│   │   └── registry.py                # FeatureGroup enum + PLAYER_SEASON_KEY + get_feature_names
│   ├── models/
│   │   ├── __init__.py
│   │   ├── utils.py                   # Shared: align_features, reconstruct_scaler, scale_and_clamp, model configs
│   │   ├── baselines/
│   │   │   ├── __init__.py
│   │   │   └── shrinkage.py           # Closed-form Beta-Binomial ROS baseline (point + quantiles)
│   │   ├── mtl/                       # Preseason MTL (point estimates)
│   │   │   ├── __init__.py
│   │   │   ├── model.py               # MTLForecaster + MTLEnsembleForecaster
...
│       ├── pa_projection.py           # Marcel PA projection + rate_to_count helper
...
└── tests/
    ...
    ├── test_in_season_features.py     # Phase 2 in-season feature matrix + trail4w derivations
    ├── test_mtl.py                    # Preseason MTL core tests
    ...
    ├── test_mtl_ros_splits.py         # walk_forward_split + leakage assertion
    ├── test_mtl_ros_train.py          # train_ros() end-to-end on synthetic snapshots
    └── test_shrinkage.py              # Shrinkage posterior math + tau fitting + quantiles
```
...
- **Data pipeline**: merge correctness, target alignment (Y+1), no temporal leakage, correct splits, weekly counting-stat coverage validation vs season totals, BRef weekly fetch fail-hard + per-week retry behavior
